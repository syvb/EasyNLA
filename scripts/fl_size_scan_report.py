...
def main(argv=None):
    ...
    for size, r in R.items():
        ks = {k for k, h in r["hits"].items() if all(x is not None for x in h)}
    ...
    L = ["# Single-layer Future Oracle size scan", "",
         ...
         "## Own set (final checkpoint)", "",
         "| size | layer | n | " + " | ".join(f"N={N} p1 / tf" for N in range(N_MAX + 1)) + " |",
         "|---|---|---|" + "---|" * (N_MAX + 1)]
    for size, r in R.items():
        cells = [f"{r['agg'][('real', N, 'p1')][0]:.3f} / {r['agg'].get(('real', N, 'tf_p1'), (float('nan'),))[0]:.3f}" for N in range(N_MAX + 1)]
        L.append(f"| {size} | {r['layer']} | {r['n_own']} | " + " | ".join(cells) + " |")
    ...
    for size, r in R.items():
        ...
        L.append(f"| {size} | **oracle L{r['layer']}** | " + " | ".join(f"**{r['real'][N]:.3f}**" for N in range(N_MAX + 1)) + " |")
    ...
    try:
        ...
        for size, r in R.items():
            ax[0].plot(Ns, [r["real"][N] for N in Ns], "o-", label=f"{size} L{r['layer']} (own set)")
            ...
            if tw:
                ...
                ax[2].axhline(r["real"][1], ls=":", color=ax[2].lines[-1].get_color())
