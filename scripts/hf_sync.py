"""Mirror KL-NLA artifacts to a private HuggingFace dataset repo, and back.

The pods that run this experiment keep no network volume (see the RunPod notes
in docs/kl_nla.md), so HuggingFace is the only durable store: every stage
uploads what it produced and the next stage pulls what it needs. Checkpoints
and the audit outputs all go to the same repo under
predictable prefixes.

    python scripts/hf_sync.py push  <repo> <local_dir> <remote_prefix>
    python scripts/hf_sync.py pull  <repo> <remote_prefix> <local_dir>
    python scripts/hf_sync.py wait  <repo> <remote_path> [--timeout-min 240]
"""
...
def push(a):
    ...
    api.upload_folder(
        ...
        commit_message=f"kl-nla: {a.remote_prefix}",
    )
